...
import com.zl.whaley_dbsetting.BatchDBSheetBuilder;
import com.zl.whaley_dbsetting.BatchHandleActivity;
import com.zl.whaley_dbsetting.ConfigActivity;
import com.zl.whaley_dbsetting.DBHandleActivity;
import com.zl.whaley_dbsetting.DBSheetBuilder;
...
//				Toast.makeText(MainActivity.this, "Result is NG! Multiple("+String.valueOf(msg.obj)+") Results Contained!", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(MainActivity.this, "扫描结果不符合要求，请重新扫描！", Toast.LENGTH_SHORT).show();
...
//					detail.setTextColor(Color.BLACK);
				...
				if(batchlist.get(1).equals("EMPTY")){
					Toast.makeText(MainActivity.this, "批次信息为空，请先建立批次存储表！", Toast.LENGTH_SHORT).show();
				}else{
					isbatched = true;
...
//				tvcount.setText(String.valueOf(dbsb.getRowsFromExcel(filename)));//使用读取Excel行数来获取本批次本机已扫描数量，
				...
				resultview.setImageDrawable(getResources().getDrawable(R.drawable.ng));
				detail.setTextColor(Color.RED);
				Toast.makeText(MainActivity.this,"此批次已锁定，不能进行操作！" , Toast.LENGTH_SHORT).show();
				soundPool.play(1,1, 1, 0, 0, 1);//播放缓存池中第一个声音，即error
				IS_ALLLOCKED = true;//锁定操作
				showErrorDialog();//弹出错误提示框
				...
				resultview.setImageDrawable(getResources().getDrawable(R.drawable.ok));
				detail.setTextColor(Color.GREEN);
				Toast.makeText(MainActivity.this,"Excel表格生成成功，本批次已锁定！" , Toast.LENGTH_SHORT).show();
				...
				resultview.setImageDrawable(getResources().getDrawable(R.drawable.ng));
				detail.setTextColor(Color.RED);
				Toast.makeText(MainActivity.this,"Excel表格生成失败，请重新生成！" , Toast.LENGTH_SHORT).show();
				soundPool.play(1,1, 1, 0, 0, 1);//播放缓存池中第一个声音，即error
				IS_ALLLOCKED = true;//锁定操作
				showErrorDialog();//弹出错误提示框
				...
				resultview.setImageDrawable(getResources().getDrawable(R.drawable.ng));
				detail.setTextColor(Color.RED);
				Toast.makeText(MainActivity.this,"请扫描之后再进行生成操作！" , Toast.LENGTH_SHORT).show();
				soundPool.play(1,1, 1, 0, 0, 1);//播放缓存池中第一个声音，即error
				IS_ALLLOCKED = true;//锁定操作
				showErrorDialog();//弹出错误提示框
				...
				detail.setTextColor(Color.RED);
				resultview.setImageDrawable(getResources().getDrawable(R.drawable.ng));
				Toast.makeText(MainActivity.this, "本批次扫描结束或批次信息未建立，请先建立批次存储表！", Toast.LENGTH_SHORT).show();
				soundPool.play(1,1, 1, 0, 0, 1);//播放缓存池中第一个声音，即error
				IS_ALLLOCKED = true;//锁定操作
				showErrorDialog();//弹出错误提示框
			}
...
//			batchlist = batchbuilder.getLatestBatch();//获取当前批次信息
			...
			if(f.exists()){
				Properties properties = ConfigActivity.loadConfig(MainActivity.this, currentbatchpath);
...
//	};
	...
	public boolean onOptionsItemSelected(android.view.MenuItem item) {
		if (item.getItemId() == 1) {
			...
			startActivity(new Intent(MainActivity.this,DBHandleActivity.class));
		} else if (item.getItemId() == 6) {
			startActivity(new Intent(MainActivity.this,ConfigActivity.class));
		} else if (item.getItemId() == 7) {
			startActivity(new Intent(MainActivity.this,BatchHandleActivity.class));
...
//    	String[] emailReciver = new String[]{"zhonglei@tcl.com"};  
    	...
    	startActivity(Intent.createChooser(email, "请选择邮件发送软件"));
    		...
    		Toast.makeText(MainActivity.this, "收件人未指定！", Toast.LENGTH_SHORT).show();
    	}
    	 }else{
    		 Toast.makeText(MainActivity.this, "指定附件不存在，是否已经进行了扫描？", Toast.LENGTH_SHORT).show();
    	 }
    }
...
//									new Thread(initBatch).start();
									...
									Toast.makeText(MainActivity.this, "数据库存储失败或批次号已存在，请重新创建！", Toast.LENGTH_SHORT).show();	
								}
							}catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
								Toast.makeText(MainActivity.this, "原表格备份失败，请重新创建新记录表！", Toast.LENGTH_SHORT).show();
							}
	                    }else{
							if(batchbuilder.saveBatch(batchname, batchsum)){
								FileUtils.copyFile(oldpath, newpath);
								...
								Toast.makeText(MainActivity.this, "数据库存储失败，请重新创建！", Toast.LENGTH_SHORT).show();	
							}
	                    }
	                    }
	                }else{
	                	Toast.makeText(MainActivity.this, "输入参数不符合要求，请重新输入！", Toast.LENGTH_SHORT).show();
	                }
	                }
	            })  
	    .setNegativeButton("取消",new DialogInterface.OnClickListener() {
...
private void showRebuildDialogWithoutExcel(){
	    ...
	    .setPositiveButton("确定",new DialogInterface.OnClickListener() {
	                ...
	                public void onClick(DialogInterface dialog,int which) { 
	                	...
	                	if((isInputCorrect(batchname))&&(!batchname.isEmpty())&&(isNumber(batchsum))&&(Integer.parseInt(batchsum)>0)&&(!batchname.equals("EMPTY"))&&(!batchname.equals("NO"))){
	                		...
	                		if(batchbuilder.saveBatch(batchname, batchsum)){
	                			Toast.makeText(MainActivity.this, "数据库写入成功，新批次已建立！", Toast.LENGTH_SHORT).show();
	                			new Thread(changeBatch).start();
	                		}else{
	                			Toast.makeText(MainActivity.this, "数据库写入失败，新批次未建立！", Toast.LENGTH_SHORT).show();
	                		}
	               
	                }else{
	                	Toast.makeText(MainActivity.this, "输入参数不符合要求(不能包含除大小写字母及数字以外的字符)，请重新输入！", Toast.LENGTH_SHORT).show();
	                }
	                }
	            })  
	    .setNegativeButton("取消",new DialogInterface.OnClickListener() {
	...
	public void showBatchList(final String[ ] str){//弹出批次列表
		new AlertDialog.Builder(MainActivity.this).setTitle("选择批次") 
    ...
    .setPositiveButton("确定",new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog,int which) { 
                	...
                	if((batchbuilder.updateFlag(batch, isComplete))>0){
                		...
                		Toast.makeText(MainActivity.this, "批次信息更新失败！", Toast.LENGTH_SHORT).show();
                	}
                }
            })  
    .setNegativeButton("取消",new DialogInterface.OnClickListener() {
...
private void showOutBoundDialog(){
    ...
    .setPositiveButton("确定",new DialogInterface.OnClickListener() {
                ...
                Toast.makeText(MainActivity.this, "已将扫描状态设置为出库操作状态！", Toast.LENGTH_SHORT).show();
                }
            })  
    .setNegativeButton("取消",new DialogInterface.OnClickListener() {
                @Override  
...
private void showRevocationDialog(){
    ...
    .setPositiveButton("确定",new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog,int which) { 
                    ...
                    Toast.makeText(MainActivity.this, "已将扫描状态设置为撤销操作状态！", Toast.LENGTH_SHORT).show();
                }
            })  
    .setNegativeButton("取消",new DialogInterface.OnClickListener() {
                @Override  
...
//	    	     }
        ...
        private void checkNetState(){
        	boolean netstate = isNetworkAvailable(MainActivity.this);
            if(netstate){
          	  Message msg = Message.obtain(mainhandler);
          	  msg.what = 0x06;
          	  msg.sendToTarget();
...
//	      Toast.makeText(Main.this,"输入的是数字", Toast.LENGTH_SHORT).show();
	...
	private void saveBatch(){
		...
		ConfigActivity.saveConfig(MainActivity.this, currentbatchpath, properties);
	}
