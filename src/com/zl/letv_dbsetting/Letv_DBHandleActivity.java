...
//    	String[] emailReciver = new String[]{"zhonglei@tcl.com"};
    	...
    	startActivity(Intent.createChooser(email, "请选择邮件发送软件"));
    		...
    		Toast.makeText(Letv_DBHandleActivity.this, "收件人未指定！", Toast.LENGTH_SHORT).show();
    	}
    	 }else{
    		 Toast.makeText(Letv_DBHandleActivity.this, "指定附件不存在，请先查询生成表格后再发送！", Toast.LENGTH_SHORT).show();
    	 }
    }
...
//				}
	...
	/*private Runnable batchRevocation = new Runnable(){
		...
		public void run() {
    		...
    		if(isConnected){
    		String sqlupdate = "update LETV_SN_MAC set outbound = 'NO' where  OQC = '"+batchname+"'";//更新出库标志位YES
    		JtdsStatement st;
	                    ...
	                    // TODO Auto-generated method stub  
	                	 ...
	                	 if(!batch.isEmpty()){
	                		 ...
	                		 new Thread(batchQuery).start();
	                			 ...
	                			 Toast.makeText(Letv_DBHandleActivity.this, "输入批次已锁定或不存在！", Toast.LENGTH_SHORT).show();
	                		 }
	                	 }else{
	                		 Toast.makeText(Letv_DBHandleActivity.this, "请输入批次号！", Toast.LENGTH_SHORT).show();
	                	 }
