package com.zl.whaley_dbsetting;
...
public class DBHandleActivity extends Activity{
...
//    	String[] emailReciver = new String[]{"zhonglei@tcl.com"};
    	...
    	startActivity(Intent.createChooser(email, "请选择邮件发送软件"));
    		...
    		Toast.makeText(DBHandleActivity.this, "收件人未指定！", Toast.LENGTH_SHORT).show();
    	}
    	 }else{
    		 Toast.makeText(DBHandleActivity.this, "指定附件不存在，请先查询生成表格后再发送！", Toast.LENGTH_SHORT).show();
    	 }
    }
...
//				}
	...
	private Runnable batchRevocationWAN = new Runnable(){
		...
		public void run() {
			// TODO Auto-generated method stub
			if(dbUtils.setDefaultOutbondState(batchname)){
				 mDbHelper.deleteWithBatch("family_bill", batchname);
				 Message msg = Message.obtain(mhandler);
				 msg.what = 0x10;//远程数据库操作成功
				 msg.sendToTarget();
	...
	private void showBatchQueryDialog(){
		...
		final EditText edtinput = new EditText(DBHandleActivity.this); 
		
		new AlertDialog.Builder(DBHandleActivity.this).setIcon(android.R.drawable.btn_star)  
	                    ...
	                    // TODO Auto-generated method stub  
	                	 ...
	                	 if(!batch.isEmpty()){
	                		 ...
	                		 new Thread(batchQuery).start();
	                			 ...
	                			 Toast.makeText(DBHandleActivity.this, "输入批次已锁定或不存在！", Toast.LENGTH_SHORT).show();
	                		 }
	                	 }else{
	                		 Toast.makeText(DBHandleActivity.this, "请输入批次号！", Toast.LENGTH_SHORT).show();
	                	 }
