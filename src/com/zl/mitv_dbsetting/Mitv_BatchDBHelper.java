...
public class Mitv_BatchDBHelper extends SQLiteOpenHelper {

	public static final String DB_NAME = "ldm_batch_mitv"; // DB name
	private Context mcontext;
	private static Mitv_BatchDBHelper mBatchDbHelper;
	private SQLiteDatabase db;

	public Mitv_BatchDBHelper(Context context) {
		super(context, DB_NAME, null, 13);//数据库版本号由11-->12，初始化时会调用onUpgrade方法
		this.mcontext = context;
	}
	
	public synchronized static Mitv_BatchDBHelper getInstance(Context context) { //单例模式获取数据库对象
		if (mBatchDbHelper == null) { 
			mBatchDbHelper = new Mitv_BatchDBHelper(context); 
		}
		return mBatchDbHelper; 
		};

	public Mitv_BatchDBHelper(Context context, String name, CursorFactory factory, int version) {
...
//		String sql = "create table if not exists batch_list_mitv(id integer primary key,create_time text,batch_name text,batch_size text)";
		/*---------------创建批次信息表，包含的列有：create_time（数据表创建时间）、batch_name（批次名称）、-------------------------------
		-----------------batch_size（本批次机器总量）、scan_flag（是否已经扫描完成）---------------------------------------------------------------*/
		String sql = "create table if not exists batch_list_mitv(id integer primary key,create_time text,batch_name text,batch_size text,scan_flag text)";
		db.execSQL(sql);
	}

	@Override
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
//		db.execSQL("ALTER TABLE batch_list_mitv ADD  scan_flag TEXT DEFAULT 'NO';");
	}

	/* 打开数据库,如果已经打开就使用，否则创建 */
	public Mitv_BatchDBHelper open() {
		if (null == mBatchDbHelper) {
			mBatchDbHelper = new Mitv_BatchDBHelper(mcontext);
		}
...
//		return db.delete("batch_list_mitv", "scan_flag = ?", new String[]{"NO"});
		return 1;
	}
	...
	public int updateSum(String batchname,String sum){//更新数据库内的扫描总数
		...
		return db.update("batch_list_mitv", values, "batch_name=? ", new String[]{batchname});
	}
