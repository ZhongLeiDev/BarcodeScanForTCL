...
import com.zl.mitv_soap.Mitv_DBUtils;

public class Mitv_DBHandleActivity extends Activity{
	...
	private File file;
	private String excelpath = Mitv_DBSheetBuilder.getSDPath() + File.separator+"ScanResultData"+File.separator+"Query"+File.separator+"mitv_query.xls";
	private String[] title = {"出货时间", "整机SN", "有线网MAC","生产日期","栈板号"};//小米存储Excel表表头
	...
	private Handler mhandler = new Handler(){
		...
		public void handleMessage(Message msg){
			if(msg.what == 0x01){
				...
				ada.notifyDataSetChanged();
				tvshowdetail.setText("已查询到"+String.valueOf(msg.obj)+"条数据！");
				tvshowdetail.setTextColor(Color.BLACK);
				...
				tvshowdetail.setText("远程数据库操作失败！");
				tvshowdetail.setTextColor(Color.RED);
				mydialog.cancel();
			}
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDbHelper = new Mitv_DBHelper(this);
		mDbHelper.open();//打开数据库
		
		batchbuilder = new Mitv_BatchDBSheetBuilder(this);
	...
	private OnClickListener click = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(v == btnquery){
				...
				//-------------------------------->对用户隐藏，避免误操作<-------------------------------------
//				int number = mDbHelper.deleteAll("family_bill_mitv");
...
//    	String[] emailReciver = new String[]{"zhonglei@tcl.com"};
    	...
    	startActivity(Intent.createChooser(email, "请选择邮件发送软件"));
    		...
    		Toast.makeText(Mitv_DBHandleActivity.this, "收件人未指定！", Toast.LENGTH_SHORT).show();
    	}
    	 }else{
    		 Toast.makeText(Mitv_DBHandleActivity.this, "指定附件不存在，请先查询生成表格后再发送！", Toast.LENGTH_SHORT).show();
    	 }
    }
	...
	private Runnable queryRunnable = new Runnable(){
		...
		public void run() {
			...
			queryresultlist.add(new QueryResult("出库时间", "SN码"));
			if(isstart){
				Cursor cur = mDbHelper.exeSql("select * from family_bill_mitv limit 100");
...
//				ada.notifyDataSetChanged();
				...
				Cursor cur = mDbHelper.exeSql("select * from family_bill_mitv where Outbound_time > '"+edtstart.getText().toString()+" 00:00:00"+"' and Outbound_time < '"+edtstop.getText().toString()+" 23:59:59"+"'");
...
//				}
	...
	private Runnable batchQuery = new Runnable(){
		...
		public void run() {
			// TODO Auto-generated method stub
			Cursor cur = mDbHelper.exeSql("select * from family_bill_mitv where batch = '"+batchname+"'");
	...
	private Runnable batchRevocationWAN = new Runnable(){
		...
		public void run() {
			// TODO Auto-generated method stub
			if(dbUtils.setDefaultOutbondState_Mitv(batchname)){
				 mDbHelper.deleteWithBatch("family_bill_mitv", batchname);
				 Message msg = Message.obtain(mhandler);
	...
	private void showBatchQueryDialog(){
		...
		final EditText edtinput = new EditText(Mitv_DBHandleActivity.this); 
		
		new AlertDialog.Builder(Mitv_DBHandleActivity.this).setIcon(android.R.drawable.btn_star)  
	                    ...
	                    // TODO Auto-generated method stub  
	                	 ...
	                	 if(!batch.isEmpty()){
	                		 ...
	                		 new Thread(batchQuery).start();
	                			 ...
	                			 Toast.makeText(Mitv_DBHandleActivity.this, "输入批次已锁定或不存在！", Toast.LENGTH_SHORT).show();
	                		 }
	                	 }else{
	                		 Toast.makeText(Mitv_DBHandleActivity.this, "请输入批次号！", Toast.LENGTH_SHORT).show();
	                	 }
...
//		beanList.add(cur.getString(4));//批次号，暂时不需要
		mylist.add(beanList);
	}
	
	public boolean saveQueryDataToExcel(ArrayList<ArrayList<String>> list) {
		file = new File(Mitv_DBSheetBuilder.getSDPath() + File.separator+"ScanResultData"+File.separator+"Query");
		Mitv_DBSheetBuilder.makeDir(file);
