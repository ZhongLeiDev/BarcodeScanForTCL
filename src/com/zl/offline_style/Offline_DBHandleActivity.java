...
public class Offline_DBHandleActivity extends Activity{
	...
	private File file;
	private String excelpath = Offline_DBSheetBuilder.getSDPath() + File.separator+"ScanResultData"+File.separator+"Query"+File.separator+"query.xls";
	private String[] title = {"Outbound_time", "SERIAL_NUMBER", "MAC"};//乐视存储Excel表表头
	private ArrayList<ArrayList<String>> mylist = new ArrayList<ArrayList<String>>();//数据存储列表
	private static final String Properfile = Environment.getExternalStorageDirectory()+ File.separator+"ScanResultData"+File.separator+"Config"+File.separator+"offline_config.dat";//联系人列表
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDbHelper = new Offline_DBHelper(this);
		mDbHelper.open();//打开数据库
		
		batchbuilder = new Offline_BatchDBSheetBuilder(this);
	...
	private OnClickListener click = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(v == btnquery){
				...
				//-------------------------------->对用户隐藏，避免误操作<-------------------------------------
//				int number = mDbHelper.deleteAll("family_bill_offline");
...
//    	String[] emailReciver = new String[]{"zhonglei@tcl.com"};
    	...
    	startActivity(Intent.createChooser(email, "请选择邮件发送软件"));
    		...
    		Toast.makeText(Offline_DBHandleActivity.this, "收件人未指定！", Toast.LENGTH_SHORT).show();
    	}
    	 }else{
    		 Toast.makeText(Offline_DBHandleActivity.this, "指定附件不存在，请先查询生成表格后再发送！", Toast.LENGTH_SHORT).show();
    	 }
    }
	...
	private Runnable queryRunnable = new Runnable(){
		...
		public void run() {
			...
			queryresultlist.add(new QueryResult("出库时间", "SN码"));
			if(isstart){
				Cursor cur = mDbHelper.exeSql("select * from family_bill_offline limit 100");
...
//				ada.notifyDataSetChanged();
				...
				Cursor cur = mDbHelper.exeSql("select * from family_bill_offline where Outbound_time > '"+edtstart.getText().toString()+" 00:00:00"+"' and Outbound_time < '"+edtstop.getText().toString()+" 23:59:59"+"'");
...
//				ada.notifyDataSetChanged();
	...
	private Runnable batchQuery = new Runnable(){
		...
		public void run() {
			// TODO Auto-generated method stub
			Cursor cur = mDbHelper.exeSql("select * from family_bill_offline where batch = '"+batchname+"'");
	 ...
	 * 离线批次撤销
	 */
	private Runnable batchRevocationOffline = new Runnable(){

		@Override
		public void run() {
			// TODO Auto-generated method stub
				 mDbHelper.deleteWithBatch("family_bill_offline", batchname);
				 Message msg = Message.obtain(mhandler);
	...
	private void showBatchQueryDialog(){
		...
		final EditText edtinput = new EditText(Offline_DBHandleActivity.this); 
		
		new AlertDialog.Builder(Offline_DBHandleActivity.this).setIcon(android.R.drawable.btn_star)  
	                    ...
	                    // TODO Auto-generated method stub  
	                	 ...
	                	 if(!batch.isEmpty()){
	                		 ...
	                		 new Thread(batchQuery).start();
	                			 ...
	                			 Toast.makeText(Offline_DBHandleActivity.this, "输入批次已锁定或不存在！", Toast.LENGTH_SHORT).show();
	                		 }
	                	 }else{
	                		 Toast.makeText(Offline_DBHandleActivity.this, "请输入批次号！", Toast.LENGTH_SHORT).show();
	                	 }
...
private void showRevocationDialog(){
	    ...
	    .setPositiveButton("确定",new DialogInterface.OnClickListener() {
	                ...
	                public void onClick(DialogInterface dialog,int which) { 
	                	...
	                	if(isUnlockedAndSuite(batch)){
	                	mydialog.show();
	                	new Thread(batchRevocationOffline).start();
	                	}else{
	                		Toast.makeText(Offline_DBHandleActivity.this, "输入批次已锁定或不存在！", Toast.LENGTH_SHORT).show();
	                	}
	                	}else{
...
//		beanList.add(cur.getString(4));//批次号，暂时不需要
	...
	public boolean saveQueryDataToExcel(ArrayList<ArrayList<String>> list) {
		file = new File(Offline_DBSheetBuilder.getSDPath() + File.separator+"ScanResultData"+File.separator+"Query");
		Offline_DBSheetBuilder.makeDir(file);
